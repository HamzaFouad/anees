...
from ui.theme import PRIMARY, FG, FG_MUTED, BG, BG_MUTED, BORDER, EQUALIZER_SVG
...
class Toolbar(QWidget):
    ...
    def __init__(self, state: AppState, parent=None):
        ...
        name = QLabel("Anees")
        name.setStyleSheet(f"font-size:14px; font-weight:700; letter-spacing:-0.01em; color:{FG};")
        lay.addWidget(name)
        lay.addWidget(VSep())
        ...
        self._add_btn.setStyleSheet(f"""
            ...
            QPushButton:disabled {{ background:#F3F4F6; color:{FG_MUTED}; opacity:0.7; }}
        """)
...
class RunControls(QWidget):
    ...
    def refresh(self):
        ...
        if rs == RunState.IDLE:
            ...
            start_btn.setIcon(QIcon(icon_pixmap("play", 13, "#fff" if can_start else FG_MUTED)))
            start_btn.setFixedHeight(32)
            start_btn.setEnabled(can_start)
            start_btn.setCursor(Qt.PointingHandCursor)
            start_btn.setStyleSheet(f"""
                QPushButton {{
                    background:{PRIMARY if can_start else '#E5E7EB'};
                    color:{'#fff' if can_start else FG_MUTED};
                    border:none; border-radius:6px;
                    padding:0 16px; font-size:13px; font-weight:600;
                }}
                QPushButton:hover {{ background:#0039D9; }}
                QPushButton:disabled {{ cursor:not-allowed; }}
            """)
            ...
            hint.setStyleSheet(f"font-size:12px; color:{FG_MUTED};")
            ...
            stop_btn = self._ctrl_btn("x", "Stop", "#991B1B", BG, "#FECACA")
            ...
            info.setStyleSheet(f"font-size:12px; color:{FG}; font-weight:500;")
            self._lay.addWidget(info)

        elif rs == RunState.PAUSED:
            resume_btn = QPushButton("  Resume")
            resume_btn.setIcon(QIcon(icon_pixmap("play", 13, "#fff")))
            ...
            paused_lbl.setStyleSheet(f"font-size:12px; color:#92400E;")
            self._lay.addWidget(paused_lbl)

        elif rs == RunState.COMPLETE:
            merge_btn = QPushButton("  Merge to folder")
            merge_btn.setIcon(QIcon(icon_pixmap("merge", 13, "#fff")))
            ...
            done_lbl.setStyleSheet(f"font-size:12px; color:#065F46;")
